...
apt-get install python3.12 python3.12-venv python3.12-dev python3-pip -y &>/dev/null
FOLDER="rl-swarm"
if [ -d "$FOLDER" ]; then
  echo "Error: Папка '$FOLDER' уже существует.
Удалите и перезапустите скрипт." >&2
  exit 1
fi
...
# Check if the version is lower than 18.0.0
